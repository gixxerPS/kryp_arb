...
function decodeHtxMessage(data: WebSocket.RawData): any {
...
export default function startHtxDepth(): void {
  ...
  const symbols = cfg.symbols.filter((sym) => getEx(sym, 'htx')?.enabled);
  if (symbols.length === 0) {
    ...
    onOpen: async (ws) => {
      ...
      for (const sym of symbols) {
        ws.send(JSON.stringify({
          sub: `market.${getEx(sym, 'htx')!.mdKey}.mbp.refresh.${depthLevels}`,
          id: `htx-depth-${getEx(sym, 'htx')!.mdKey}`,
        }));
      }
    },
    onMessage: (msg) => {
      ...
      try {
        const parsed = decodeHtxMessage(msg);
