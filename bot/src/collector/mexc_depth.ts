...
export default function startMexcDepth(): void {
  ...
  const symbols = cfg.symbols.filter((sym: string) => getEx(sym, 'mexc')?.enabled);
  if (symbols.length === 0) {
  ...
  const params = symbols.map((sym: string) => `spot@public.limit.depth.v3.api.pb@${getEx(sym, 'mexc')!.mdKey}@${levels}`);
